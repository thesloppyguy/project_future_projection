...
class SOBCalculator:
    ...
    def __init__(self, market_share_file: Path, cutoff_date: str = "2025-03-31"):
        """
        Initialize SOB calculator.

        Args:
            market_share_file: Path to market share CSV file
            cutoff_date: Only use data up to this date (format: YYYY-MM-DD)
        """
    ...
    def load_market_share_data(self) -> pd.DataFrame:
        ...
        # Filter to only include data up to cutoff date
        df = df[df["YearMonth"] <= self.cutoff_date].copy()
        ...
        logger.info(
            f"Loaded {len(df)} months of market share data (up to {self.cutoff_date.strftime('%Y-%m-%d')})"
        )
    ...
    def calculate_historical_sob_by_fy(self, branch: str) -> pd.DataFrame:
        ...
        fy_stats = (
            df.groupby("FinancialYear")
            .agg({sob_col: ["mean", "std", "min", "max", "count"]})
            .reset_index()
        )
        ...
        if len(complete_fys) > 0:
            ...
            logger.info(
                f"  Range: {overall_min:.4f} - {overall_max:.4f} ({overall_min * 100:.2f}% - {overall_max * 100:.2f}%)"
            )
            logger.info(
                f"  Complete FYs: {sorted(complete_fys['FinancialYear'].tolist())}"
            )
    ...
    def forecast_sob(
        self, branch: str, n_periods: int = 12, model_type: str = "prophet"
    ) -> pd.Series:
        """
        ...
        # Create time series (data already filtered to cutoff_date in load_market_share_data)
        sob_series = pd.Series(df[sob_col].values, index=df["YearMonth"])

        if len(sob_series) < 12:
            logger.warning(
                f"Insufficient data for {branch} SOB forecasting, using historical mean"
            )
            historical_mean = self.historical_sob.get(branch, {}).get(
                "mean", sob_series.mean()
            )
        ...
        logger.info(
            f"Forecasting {branch} SOB for {n_periods} months using {model_type}..."
        )
        ...
        if model_type == "prophet":
            ...
            last_date = sob_series.index.max()
            future_dates = create_future_dates(last_date, n_periods, "MS")
            forecast = model_trainer.forecast_prophet(
                model, n_periods, last_date, freq="MS"
            )
            ...
            if model is None:
                logger.warning(
                    f"Holt-Winters failed for {branch}, using historical mean"
                )
                historical_mean = self.historical_sob.get(branch, {}).get(
                    "mean", sob_series.mean()
                )
            ...
            last_date = sob_series.index.max()
            forecast = model_trainer.forecast_holt_winters(
                model, n_periods, last_date, freq="MS"
            )
            ...
            last_date = sob_series.index.max()
            forecast = model_trainer.forecast_auto_arima(
                model, n_periods, last_date, freq="MS"
            )
        ...
        logger.info(
            f"  {branch} SOB forecast range: {forecast.min():.4f} - {forecast.max():.4f}"
        )
        logger.info(
            f"  {branch} SOB forecast mean: {forecast.mean():.4f} ({forecast.mean() * 100:.2f}%)"
        )

        return forecast

    def forecast_yearly_sob(self, branch: str, model_type: str = "prophet") -> float:
        """
        Forecast yearly SOB for the next financial year.

        Args:
            branch: Branch name
            model_type: Model to use (currently uses linear trend)

        Returns:
            Forecasted yearly SOB (single value)
        """
        if self.market_share_df is None:
            self.load_market_share_data()
        ...
        # Aggregate monthly SOB to yearly (average SOB per FY)
        # Group by financial year and calculate average SOB
        yearly_sob = df.groupby("FinancialYear")[sob_col].mean().sort_index()

        if len(yearly_sob) < 3:
            logger.warning(
                f"Insufficient yearly data for {branch} SOB forecasting, using historical mean"
            )
            return self.historical_sob.get(branch, {}).get("mean", yearly_sob.mean())

        logger.info(f"Forecasting {branch} yearly SOB using linear trend...")

        # For yearly data, use simple linear trend or historical mean
        # Since we have limited yearly data points, use a simple approach
        if len(yearly_sob) >= 3:
            # Use linear trend for forecasting
            years = np.array(yearly_sob.index)
            values = yearly_sob.values

            # Fit linear trend
            coeffs = np.polyfit(years, values, 1)
            # Forecast next year (last year + 1)
            next_year = yearly_sob.index.max() + 1
            yearly_sob_forecast = np.polyval(coeffs, next_year)

            # Clip to reasonable bounds (use historical min/max as bounds)
            hist_min = yearly_sob.min()
            hist_max = yearly_sob.max()
            yearly_sob_forecast = max(
                hist_min * 0.5, min(hist_max * 1.5, yearly_sob_forecast)
            )
        else:
            # Not enough data for trend, use historical mean
            yearly_sob_forecast = yearly_sob.mean()

        # Ensure SOB is between 0 and 1
        yearly_sob_forecast = max(0.0, min(1.0, yearly_sob_forecast))

        logger.info(
            f"  {branch} Forecasted Yearly SOB: {yearly_sob_forecast:.4f} ({yearly_sob_forecast * 100:.2f}%)"
        )

        return float(yearly_sob_forecast)


class ProductionForecaster:
    """Production forecasting using best models and dynamic SOB-based market estimation."""

    def __init__(self, model_name: str = "catboost", sob_model_type: str = "prophet"):
        """
        ...
        # SOB calculator (only use data up to 2025-03-31)
        market_share_file = DATA_DIR / "market_share_by_month_year.csv"
        self.sob_calculator = SOBCalculator(
            market_share_file, cutoff_date=CALIBRATION_END_DATE
        )
        ...
        # Forecasted SOB (yearly)
        self.blr_yearly_sob_forecast = None
        self.maa_yearly_sob_forecast = None
    ...
    def load_models_and_data(self):
        ...
        logger.info(
            f"BLR series: {len(self.blr_series)} periods, {self.blr_series.index.min()} to {self.blr_series.index.max()}"
        )
        logger.info(
            f"MAA series: {len(self.maa_series)} periods, {self.maa_series.index.min()} to {self.maa_series.index.max()}"
        )
        ...
        _, _, self.blr_feature_names = self.feature_engineer.prepare_ml_features(
            self.blr_series
        )
        _, _, self.maa_feature_names = self.feature_engineer.prepare_ml_features(
            self.maa_series
        )
    ...
    def calculate_and_forecast_sob(self, n_periods: int = 12):
        """Calculate historical SOB and forecast yearly SOB for next FY."""
        logger.info("=" * 80)
        ...
        all_branches = [
            col
            for col in self.sob_calculator.market_share_df.columns
        ...
        # Forecast yearly SOB for next financial year
        logger.info(f"\nForecasting yearly SOB for next financial year...")
        self.blr_yearly_sob_forecast = self.sob_calculator.forecast_yearly_sob(
            "BLR", self.sob_model_type
        )
        self.maa_yearly_sob_forecast = self.sob_calculator.forecast_yearly_sob(
            "MAA", self.sob_model_type
        )

    def generate_forecast(
        self, model: Any, series: pd.Series, feature_names: list, n_periods: int = 12
    ) -> pd.Series:
        """
        ...
        for i, future_date in enumerate(future_dates):
            ...
            if self.model_name == "catboost":
                ...
                pred = self.model_trainer.forecast_random_forest(model, feature_vector)[
                    0
                ]
            else:
    ...
    def calculate_yearly_sob(self) -> Dict[str, float]:
        ...
        for branch in ["BLR", "MAA"]:
            if branch in self.sob_calculator.historical_sob:
                ...
                logger.info(
                    f"  {branch} Yearly SOB: {yearly_sob[branch]:.4f} ({yearly_sob[branch] * 100:.2f}%)"
                )
            else:
    ...
    def calculate_monthly_distribution(self, branch: str) -> pd.Series:
        ...
        complete_fys = self.sob_calculator.historical_sob.get(branch, {}).get(
            "complete_fys", pd.DataFrame()
        )
        if len(complete_fys) == 0:
        ...
        logger.info(
            f"  {branch} monthly distribution calculated from {len(complete_fy_list)} complete FYs"
        )
    ...
    def get_monthly_distribution_pattern(self, branch: str) -> Dict[int, float]:
        ...
        if len(complete_fys) == 0:
            logger.warning(
                f"No complete FYs found for monthly distribution, using equal distribution"
            )
    ...
    def calculate_fy_sales(
        self, blr_forecast: pd.Series, maa_forecast: pd.Series
    ) -> Dict[str, Any]:
        """
        ...
        blr_df = pd.DataFrame(
            {"date": blr_forecast.index, "forecast": blr_forecast.values}
        )
        ...
        maa_df = pd.DataFrame(
            {"date": maa_forecast.index, "forecast": maa_forecast.values}
        )
        ...
        forecast_fys = sorted(
            set(blr_df["FinancialYear"].unique())
            | set(maa_df["FinancialYear"].unique())
        )

        # Use forecasted yearly SOB for BLR and MAA
        blr_yearly_sob = self.blr_yearly_sob_forecast
        maa_yearly_sob = self.maa_yearly_sob_forecast

        logger.info(f"\nForecasted Yearly SOB for next FY:")
        ...
        for fy in forecast_fys:
            ...
            total_from_blr = (
                blr_yearly_sales / blr_yearly_sob if blr_yearly_sob > 0 else 0
            )
            total_from_maa = (
                maa_yearly_sales / maa_yearly_sob if maa_yearly_sob > 0 else 0
            )
            ...
            if blr_yearly_sob > 0 and maa_yearly_sob > 0:
                ...
                weights_maa = maa_yearly_sob / (blr_yearly_sob + maa_yearly_sob)
                total_market_sales = (
                    total_from_blr * weights_blr + total_from_maa * weights_maa
                )
            else:
            ...
            logger.info(
                f"  Range: {total_market_min:,.0f} - {total_market_max:,.0f} units"
            )
            ...
            for branch, sob_data in self.sob_calculator.historical_sob.items():
                if branch not in ["BLR", "MAA"]:
                    ...
                    logger.info(
                        f"  {branch} Yearly Sales: {branch_yearly_sales:,.0f} units (SOB: {branch_yearly_sob * 100:.2f}%)"
                    )
    ...
    def run_production_forecast(
        self, n_periods: int = 12, output_path: Optional[Path] = None
        ...
        logger.info(
            f"    BLR forecast: {blr_forecast.sum():.0f} total units over {len(blr_forecast)} months"
        )
        ...
        logger.info(
            f"    MAA forecast: {maa_forecast.sum():.0f} total units over {len(maa_forecast)} months"
        )

        # Calculate FY sales using yearly SOB (this will calculate total market sales)
        ...
        # Add yearly SOB forecasts to results
        results["yearly_sob_forecast"] = pd.DataFrame(
            {
                "Branch": ["BLR", "MAA"],
                "Yearly_SOB_Forecast": [
                    self.blr_yearly_sob_forecast,
                    self.maa_yearly_sob_forecast,
                ],
            }
        )
        ...
        if output_path:
            ...
            results["blr_forecast"].to_csv(
                output_path.parent / f"{output_path.stem}_blr_forecast.csv", index=False
            )
            results["maa_forecast"].to_csv(
                output_path.parent / f"{output_path.stem}_maa_forecast.csv", index=False
            )
            results["yearly_sob_forecast"].to_csv(
                output_path.parent / f"{output_path.stem}_yearly_sob_forecast.csv",
                index=False,
            )
            results["fy_summary"].to_csv(
                output_path.parent / f"{output_path.stem}_fy_summary.csv", index=False
            )
            ...
            for fy_key, monthly_df in monthly_dist_dfs.items():
                monthly_df.to_csv(
                    output_path.parent
                    / f"{output_path.stem}_{fy_key}_monthly_distribution.csv",
                    index=False,
                )
...
def main():
    ...
    results = forecaster.run_production_forecast(
        n_periods=12,
        output_path=Path("forecast_outputs/production")
        / f"production_forecast_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )
    ...
    print(f"\nYearly SOB Forecast:")
    print(results["yearly_sob_forecast"].to_string(index=False))
