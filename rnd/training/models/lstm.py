...
def train_model(
    ...
    if len(train_data) < 30:
        print(f"Warning: Insufficient data for LSTM for {branch}")
        return None
    ...
    try:
        ...
        if use_optimization:
            try:
                hyperparams = get_hyperparameters('lstm', train_data,
                ...
                print(f"Warning: Hyperparameter optimization failed for LSTM: {e}")
                n_steps = 12
                lstm_units_1 = 50
                lstm_units_2 = 50
                dropout_rate = 0.2
        ...
        if len(X) < 10:
            print(f"Warning: Insufficient sequences for LSTM for {branch}")
            return None
        ...
        print(f"Error training LSTM for {branch}: {e}")
        return None
...
def forecast(model: dict, n_periods: int, freq: str = 'W-MON', branch: str = None) -> pd.Series:
    ...
    try:
        ...
        for i in range(n_periods):
            ...
            # Predict
            pred_normalized = lstm_model.predict(X_pred, verbose=0)[0, 0]
