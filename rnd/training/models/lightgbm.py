...
def train_model(train_data: pd.Series, branch: str, freq: str = 'W-MON',
    ...
    if len(train_data) < 20:
        print(f"Warning: Insufficient data for LightGBM for {branch}")
        return None
    ...
    try:
        ...
        if len(X) < 10:
            print(f"Warning: Insufficient valid data for LightGBM for {branch}")
            return None
        
        # Get optimized hyperparameters
        if use_optimization:
            try:
                hyperparams = get_hyperparameters('lightgbm', train_data,
                ...
                print(f"Warning: Hyperparameter optimization failed for LightGBM: {e}")
                model_params = {
                    'n_estimators': 100,
                    'max_depth': 6,
                    'learning_rate': 0.1,
        ...
        print(f"Error training LightGBM for {branch}: {e}")
        return None
...
def forecast(model: dict, n_periods: int, freq: str = 'W-MON', branch: str = None) -> pd.Series:
    ...
    try:
        ...
        for i in range(n_periods):
            ...
            # Predict
            pred = lgb_model.predict(feature_vector)[0]
        ...
        print(f"Error forecasting with LightGBM: {e}")
        return pd.Series(dtype=float)
